...
> 详情页扩展：`docs/dlt/`、`docs/pl3/`、`docs/pl5/` 分别展示各玩法最高评分3组、最新一期结构复盘和最近100期统计分析。排列3进一步分为直选、组选3、组选6。排列3/5每日开奖后更新，大乐透在开奖前一天更新；首页每个玩法展示最高评分3组。
...
## 当前排列3/福彩3D输出约定（v4.0混合模型）

生成器以逐位滚动模型为主，融合 ZIP 模型的和值、跨度、奇偶、冷热回补和两码和差信号；ZIP 信号只作低权重辅助，不使用硬杀号或硬过滤。生成器必须先输出和值、跨度、奇偶结构，再输出百位、十位、个位的热门/冷门及近100期出现次数；每个位的前两个热门和前两个冷门重点标注；最终分别输出5个热门直选和5个冷门直选。直选号码按位置保留，不输出无序组选结果。
